...
class Gaussian():
    ...
    def __init__(self,filePath):
        ...
        #self.Inputfilename=self.InputFile.split('/')[-1]
    ...
    def GasPhaseParameterCompute(self):
        print self.InputFilepath+self.Inputfilename
    ...
    def GasBindEnergy(self):
        ...
        while(length>=0):
            if(re.match('.*SCF Done.*',lines[length])!=None):
                ...
                break
            length=length-1
        print 'GasBindEnergy:'+str(self.__BindEnergy)
        #os.remove(self.InputFilepath+self.InputfilenameWithoutExt+'.log')
    def GasPhase_IonizationPotential(self):
        ...
        while(length>=0):
            if(re.match('.*SCF Done.*',lines[length])!=None):
                ...
                break
            length=length-1
        print 'IonizationPotential: '+str(self.__IonizationPotential)
    ...
    def GasPhase_EnergyProtonation(self):
        ...
        while(length>=0):
            if(re.match('.*SCF Done.*',lines[length])!=None):
                ...
                break
            length=length-1
        print 'ElectronProtonation: '+str(self.__ElectronProtonation)
    ...
    def Gaussian_electronstaticpotential_chkTofchk(self):              
            try:
                shutil.move(globalpath+self.InputfilenameWithoutExt+'.chk', self.InputFilepath+self.InputfilenameWithoutExt+'.chk')
            except:
                print 'no file of .ck to move'

            Cmd='formchk '+self.InputFilepath+self.InputfilenameWithoutExt+'.chk'                            
            subprocess.Popen(Cmd,shell=True)
    ...
    def Gaussian_electronstaticpotential_parameterCompute(self):
        ...
        #remove file in src
        try:
            shutil.move(self.InputfilenameWithoutExt+'_density.cub',self.InputFilepath+self.InputfilenameWithoutExt+'_density.cub')
            shutil.move(self.InputfilenameWithoutExt+'_potential.cub',self.InputFilepath+self.InputfilenameWithoutExt+'_potential.cub')
        except Exception, e:
